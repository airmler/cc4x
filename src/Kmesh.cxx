...
kMesh::kMesh(std::vector<int64_t> _mesh) {
  ...
  mesh = _mesh;
  Nk =
      std::accumulate(mesh.begin(), mesh.end(), 1L, std::multiplies<int64_t>());
}

int64_t kMesh::backfold(const int64_t i, const int64_t N) {
  return (i + 4 * N) % N;
}
...
int64_t kMesh::idxMinusIdx(const int64_t i, const int64_t j) {
  ...
  std::vector<int64_t> d({(kj[0] - ki[0] + mesh[0]),
                          (kj[1] - ki[1] + mesh[1]),
                          (kj[2] - ki[2] + mesh[2])});
...
int64_t kMesh::getForthIdx(const int64_t k, const int64_t i, const int64_t j) {
  ...
  std::vector<int64_t> d({(ki[0] + kj[0] - kk[0] + mesh[0]),
...
int64_t kMesh::getMinusIdx(const int64_t i) {
  ...
  std::vector<int64_t> d({-k[0] + mesh[0], -k[1] + mesh[1], -k[2] + mesh[2]});
  return kToIdx(backfold(d));
}
