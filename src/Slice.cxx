...
sliceDim getDimensions(std::vector<int64_t> lens, std::vector<int64_t> pp) {
  ...
  std::vector<std::vector<int64_t>> dstBegin(srcBegin.size(),
...
void run(input const &in, output &out) {
  ...
  if (slices.dstBegin.size() == 1) {
    THROW("Nothing to slice. Wrong input!");
  }
  if (slices.dstBegin.size() > 4) {
    THROW("We cannot slice into more that 4 objects");
  }

  auto dummyA = new tensor<Complex>(order,
                                    slices.dstEnd[0],
                                    cc4x::kmesh->getNZC(order),
                                    cc4x::world,
                                    "Slice1");
  dummyA->slice(slices.dstBegin[0],
                slices.dstEnd[0],
                0.0,
                *in.I,
                slices.srcBegin[0],
  ...
  *out.A = dummyA;
  ...
  auto dummyB = new tensor<Complex>(order,
                                    slices.dstEnd[1],
                                    cc4x::kmesh->getNZC(order),
                                    cc4x::world,
                                    "Slice2");
  dummyB->slice(slices.dstBegin[1],
                slices.dstEnd[1],
                0.0,
                *in.I,
                slices.srcBegin[1],
  ...
  *out.B = dummyB;
  ...
  if (slices.dstBegin.size() == 2)
    return;
  auto dummyC = new tensor<Complex>(order,
                                    slices.dstEnd[2],
                                    cc4x::kmesh->getNZC(order),
                                    cc4x::world,
                                    "Slice3");
  dummyC->slice(slices.dstBegin[2],
                slices.dstEnd[2],
                0.0,
                *in.I,
                slices.srcBegin[2],
  ...
  *out.C = dummyC;
  ...
  auto dummyD = new tensor<Complex>(order,
                                    slices.dstEnd[3],
                                    cc4x::kmesh->getNZC(order),
                                    cc4x::world,
                                    "Slice4");
  dummyD->slice(slices.dstBegin[3],
                slices.dstEnd[3],
                0.0,
                *in.I,
                slices.srcBegin[3],
