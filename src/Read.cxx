...
namespace Read{
  ...
  yamlData readYaml(YAML::Node yamlFile){
    ...
    if (yamlFile["metaData"]){
      auto meta = yamlFile["metaData"];
      if (meta["No"]) y.No = meta["No"].as<int64_t>();
      if (meta["Nv"]) y.Nv = meta["Nv"].as<int64_t>();
      if (meta["kMesh"]) y.kMesh = meta["kMesh"].as< std::vector<int64_t> >();
  ...
  void run(input const& in, output& out){
    ...
    auto d = new tensor<Complex>(
      y.order, y.lens, cc4x::kmesh->getNZC(y.order), cc4x::world, "fileIO"
    );
    ...
    if (y.fileType == "TextFile"){
      ...
      if (cc4x::world->rank)  d->write(0, idx, data);
      else                 d->write(Np, idx, data);
    } else {
      MPI_File file;
      MPI_File_open(
        cc4x::world->comm(), dataFile.c_str(), MPI_MODE_RDONLY, MPI_INFO_NULL, &file
      );
