...
void run(input const &in, output &out) {
  ...
  for (int64_t g1(-maxG); g1 <= maxG; g1++)
    for (int64_t g2(-maxG); g2 <= maxG; g2++)
      for (int64_t g3(-maxG); g3 <= maxG; g3++) iGrid.push_back({g1, g2, g3});

  sort(iGrid.begin(), iGrid.end(), [](iarr a, iarr b) {
    return sL(a) < sL(b);
  });
  if (iGrid.size() < Np)
    LOG() << "BUG related to Np & maxG\n";
  ...
  for (auto &d : dGrid) {
    ...
    double exchE(0.0);
    for (int64_t o(0); o < No; o++) exchE += Vijji(d, dGrid[o], v);
    d[3] -= exchE;
  }
  double refE(0.0);
  ...
  auto eps =
      new tensor<Complex>(1, {Np}, cc4x::kmesh->getNZC(1), cc4x::world, "eps");

  for (int64_t k(0); k < _Nk; k++)
    for (int64_t d(0); d < Np; d++) energies[d + k * Np] = {dGrid[d][3], 0.0};

  std::vector<int64_t> idx(Np);
  if (!cc4x::world->rank)
    std::iota(idx.begin(), idx.end(), 0);
  if (cc4x::world->rank)
    eps->write(0, idx, energies);
  else
    eps->write(Np, idx, energies);
  ...
  *out.eps = eps;
  ...
  for (int64_t p(0); p < Np; p++)
    for (int64_t q(0); q < Np; q++) {
      iarr d = {iGrid[p][0] - iGrid[q][0],
                ...
                iGrid[p][2] - iGrid[q][2]};
      maxMom =
          std::max(maxMom, d, [](iarr a, iarr b) { return sL(a) < sL(b); });
    }
  ...
  for (auto k(0); k < _Nk; k++) cV->write(idx.size(), idx, vData, k);
